#!/bin/bash
#SBATCH --job-name=MPI2_Tests
#SBATCH --nodes=8
#SBATCH --cpus-per-task=32
#SBATCH --output=bin/mpi2_%j.log
#SBATCH --error=bin/mpi2_%j.err
...
OUTPUT_FILE_1="output/mpi2_1GB"
OUTPUT_FILE_5="output/mpi2_5GB"
OUTPUT_FILE_10="output/mpi2_10GB"
...
for i in ${!DATASETS[@]}; do
    echo "Starting MPI2 tests on ${FILE_SIZES[$i]} file.." >> "${OUTPUT_FILES[$i]}"
    echo "" >> "${OUTPUT_FILES[$i]}"
    
    # Loop sulle configurazioni dei nodi
    for N in "${NODES[@]}"; do
        echo "Configuration $N nodes:" >> "${OUTPUT_FILES[$i]}"
        srun --mpi=pmix --nodes=$N ./mpi2 "${DATASETS[$i]}" >> "${OUTPUT_FILES[$i]}" 2>&1
        echo "" >> "${OUTPUT_FILES[$i]}"
    done
