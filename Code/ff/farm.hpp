 ...
 *  not tasks. It is composed by: Emitter (E), Workers (W), Collector_Stage2 (C).
 ...
 *  are sent to Collector_Stage2 that gather them and output them in the output stream.

 *  Dispatching policy can be configured in the Emitter. Gathering policy in the
 *  Collector_Stage2.
 * 
 *  In case of no output stream the Collector_Stage2 is usually not needed. Emitter
 ...
 *
 *  \brief The Farm skeleton, with Emitter (\p lb_t) and Collector_Stage2 (\p gt_t).
 *
 ...
 *  the last (optional) stage would be the \a Collector_Stage2 (\ref ff_gatherer
...
protected:
    ...
    inline int prepare() {
        ...
        for(size_t i=0;i<nworkers;++i) {
            ...
            ff_a2a* a2a_last  = nullptr;        // the Collector_Stage2 sees this all-to-all
...
public:
     ...
     * @param Collector_Stage2 pointer to Collector_Stage2 object (optional)
    ...
    ff_farm(const std::vector<ff_node*>& W, ff_node *const Emitter=NULL, ff_node *const Collector=NULL, bool input_ch=false):
        ...
        // add default collector even if Collector_Stage2 is NULL,
     ...
     *  \param fixedsize = true uses only fixed size queue (both between Emitter and Workers and between Workers and Collector_Stage2)
     */
     ...
     *
     *  It adds the Collector_Stage2 filter to the farm skeleton. If no object is
     ...
     *  than one collector. 
     *
     *  \param c Collector_Stage2 object
     *
    ...
    int add_collector(ff_node * c, bool cleanup=false) { 
        if (c && (c->isFarm() || c->isAll2All() || c->isPipe())) {
            error("FARM, add_collector: wrong kind of node, the Collector_Stage2 filter can be either a standard node or a multi-input node or a multi-output node\n");
            return -1;
        }
     ...
     *
     * This method allows to estabilish a feedback channel from the Collector_Stage2
    ...
    /**
     * \internal
     * \brief Gets gt (Collector_Stage2) node
     *
...
protected:
     ...
     *
     *  This function redefines the ff_node's virtual method of the same name.
     *  It create an output buffer for the Collector_Stage2
     *
     *  \param nentries the size of the buffer
     *  \param fixedsize flag to decide whether the buffer is resizable. 
     *  Default is \p false
     ...
     *
     *  This function redefines the ff_node's virtual method of the same name.
     *  Set the output buffer for the Collector_Stage2.
     *
...
public:    
    ...
    //       All workers, the Emitter and the Collector_Stage2 will be deleted in the ff_Farm destructor !
