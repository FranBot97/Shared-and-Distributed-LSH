 ...
 *                   no more than chunk iterations at a time is assigned to one Workers_Stage1, the
 ...
 * this file).  Useful when there is just 
     ...
     * efficient because the Workers_Stage1 threads are created once and then
     * re-used many times.
     * On the contrary, the one-shot version has a lower setup overhead but 
     * Workers_Stage1 threads are destroyed at the end of the loop.
...
public:
     ...
     * blocks of minimal size <b>grain</b>.
     * Iteration space is walked with stride <b>step</b>. <b>thid</b> Workers_Stage1 thread ID
...
public:
     ...
     * Dynamic scheduling onto nw worker threads. Iterations are scheduled in
     * blocks of minimal size \p grain.
     * Iteration space is walked with stride \p step. \p thid Workers_Stage1 thread ID
     * is made available via a Lambda parameter.
     *
     * @param first first value of the iteration variable
     * @param last last value of the iteration variable
