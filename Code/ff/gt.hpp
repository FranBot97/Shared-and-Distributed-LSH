 ...
 *
 *  \brief Farm Collector_Stage2 (it is not a ff_node)
 *
 * It Contains the \p ff_gatherer class and methods which are used to model the \a
 *  Collector_Stage2 node, which is optionally used to gather tasks coming from
 *  workers.
 *
 ...
 *  \ingroup building_blocks
 *
 *  \brief A class representing the \a Collector_Stage2 node in a \a Farm skeleton.
 *
 *  This class models the \p gatherer, which wraps all the methods and
 *  structures used by the \a Collector_Stage2 node in a \p Farm skeleton. The \p farm
 ...
 *  and - optionally - a \p ff_gatherer called \a collector. The \a Collector_Stage2
